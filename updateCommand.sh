...
# Sources
source "${SCRIPT_DIR}/utils/requirments/updateCommand.req.sh"
...
if ! is_in_path "${LOCAL_BIN}"; then
    if [ ! -d "${LOCAL_BIN}" ]; then
        if ! ask_yn "Le répertoire '${LOCAL_BIN}' n'existe pas. Faut-il le créer et l'ajouter au PATH ?"; then
            eout "Annulation de l'utilisateur, le répertoire '${LOCAL_BIN}' n'existe pas."
        fi

        lout "Création du répertoire ${LOCAL_BIN}"
        mkdir -p "${LOCAL_BIN}" || eout "Impossible de créer le répertoire '${LOCAL_BIN}'"

        lout "Ajout du répertoire '${LOCAL_BIN}' dans le PATH"
        echo "PATH=\"\$PATH:${LOCAL_BIN}\"" >> "${BASH_PROFILE}" || eout "Impossible de modifier le PATH dans ${BASH_PROFILE}"
...
if [ "${TEST_ALIAS}" != "${test_alias_token}" ]; then
    ...
    echo "source \"${BASH_ALIASES}\"" >> "${HOME}/.bashrc" || eout "Impossible d'ajouter ${BASH_ALIASES} au .bashrc. Ajoutez la ligne de code : 'source \"${BASH_ALIASES}\"' à votre ~/.bashrc"
fi
